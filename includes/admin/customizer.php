...
function tk_google_fonts_customizer(){ ?>
	...
	<p><a href="<?php echo esc_url( get_admin_url() ); ?>customize.php"  class="button-primary">Go to the Customizer</a></p>
...
function tk_google_fonts_customize_register( $wp_customize ) {
	...
	if ( ! $tk_google_pro ) {
		...
		$wp_customize->add_control(
			new WP_Customize_TK_Google_GO_PRO_Control(
				$wp_customize,
				'site_title_pro_options',
				array(
...
function tk_google_fonts_customize_preview_init() {
	...
	wp_enqueue_script( 'google_fonts_customize_preview_js', plugins_url( '/js/customizer.js', __FILE__ ), array( 'jquery', 'customize-preview' ), '1.0', true );
				...
				<ul>
					...
					if ( is_array( $defaults ) ) {
						...
						<li data-id="<?php echo esc_attr( $key ); ?>">
							<span title="<?php echo esc_attr( $default ); ?>"><?php echo esc_attr( $default ); ?></span>
						</li>
						<?php } ?>
					<?php } ?>
